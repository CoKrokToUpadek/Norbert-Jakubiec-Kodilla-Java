package com.kodilla.stream.porfolio;
...
class BoardTestSuite {
    ...
    private Board prepareTestData() {
        ...
        User user2 = new User("project_manager1", "Nina White");
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        OptionalDouble averageOD=project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(t->t.getTasks().stream())
                .map(r-> {
                    LocalDate startDate=r.getCreated();
                    LocalDate today=LocalDate.now();
                    return (double) ChronoUnit.DAYS.between(startDate,today);
                }).mapToDouble(p->p).average();
        double ih8OptionalDouble =averageOD.orElse(-1);

        //Then
        Assertions.assertEquals(10, ih8OptionalDouble);
    }
